            ...
            for i in lst:
                if i[0] == email:
                    if i[1] == pswd1:
                        ...
                        return render_template('admdash.html', name=session["admname"] , req=r, labels=labels, values=values)
                    else:
                        return "Incorrect Password"
                else:
                    return "user doesn't exist"
...
def dash():
    ...
    try:
        ...
        if request.form.get('adm') == "Grant Access":
            ...
            # mail.send(msg)
            print("*" * 30, 'Email Sent')
        return render_template("admdash.html", labels=labels, values=values, req=r)
...
def house():
    if request.method == 'POST':
        ...
        print("Type of mrthod:", request.method)
